...
public class Screen3 extends JFrame {
	...
	/**
	 * Constructor for screen 3 that has the game mechanics and the layout and dimensions.
	 */
	...
	public Screen3() {
		...
		//splits out the question and the options
		...
		if(change==1)
		{
			...
			option1.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg){
					...
					// if the target selected is 5000$ so it start counting score else score remains 0
					if(Target.sc==2 && Target.c != 1)
					{
						Target.c = 1;
						Screen1.score = 5000;
					}
					...
					//sets the target.c == 1 (selected) so that it starts counting score for other questions
					if (Target.c == 1)
					{
						Screen1.score = 5000;
					}
					time.stop();
				  	Screen4 s=new Screen4();
					s.setVisible(true);
					close();
			...
			//if user anwers wrong answer or selects wrong option for other 3
			 ...
			 option2.addActionListener(new ActionListener(){
					public void actionPerformed(ActionEvent arg){	
						//if users answers wrong and target.c is not equal to 1 then score is o 
						if(Target.c != 1)
	        			{
							Screen1.score = 0;
	        			}
	        			...
	        			{
	        				//then check for the target and set the score according to it
	        				if(Target.sc==1)
	        				{
	    						Screen1.score = 1000;
	        				}
	        			}
						//message that displays if answer is wong with score
	    				JOptionPane.showMessageDialog(null, "Sorry your answer was wrong. You have won " + Screen1.score);
						int res = JOptionPane.showOptionDialog(null, "Would you like to play again?", "Wrong answer", JOptionPane.ERROR_MESSAGE,
		        		        JOptionPane.INFORMATION_MESSAGE, null, null, null);
						// if user wants to play again so it resets everything
		        		if(res==0){
		    				Leaderboard.addScore(Screen1.score);
		        			Screen1 s=new Screen1();
		        			s.setVisible(true);
		...
		if(change==2)
		{
			//if user selects option 1 then it is wrong and ask user to pay again with its score
			...
			option1.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg){
					...
					int res = JOptionPane.showOptionDialog(null, "Would you like to play again?", "Wrong answer", JOptionPane.ERROR_MESSAGE,
	        		        JOptionPane.INFORMATION_MESSAGE, null, null, null);
					//resets the game if he wants to play again
	        		if(res==0){
	    				Leaderboard.addScore(Screen1.score);
	        			Screen1 s=new Screen1();
	        			s.setVisible(true);
	        		...
	        		//else closes the game sending it to leader board
	        		else{
	    				Leaderboard.addScore(Screen1.score);
	        			System.exit(0);
	        		}
			...
			option2.addActionListener(new ActionListener(){
				public void actionPerformed(ActionEvent arg){
					//checks for the target and sets the score according to it
					if(Target.sc==2 && Target.c != 1)
					{
						Target.c = 1;
						Screen1.score = 5000;
